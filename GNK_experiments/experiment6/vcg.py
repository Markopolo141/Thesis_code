...
def calc_Solution(ppc, debug=False):
...
def run(input_file,output_file):
	...
	ppcs = [copy(ppc) for i in range(N)]
	for i in range(N):
		ppcs[i]['gen'][i][8] = 0
		ppcs[i]['gen'][i][9] = 0

	r = [float(rr) for rr in calc_Solution(ppc,False)]
	sum_r = sum(r)
	ppcs_r = [sum([float(rr) for rr in calc_Solution(ppc_i,False)]) for ppc_i in ppcs]

	VCG_utilities = [sum_r-ppcs_r[i] for i in range(len(ppcs))]
	#print r
	VCG_utilities_r = sum([rr**2 for rr in r])
	if VCG_utilities_r>0:
		output_file.write("{} {}\n".format(N,VCG_utilities))
	output_file.close()
