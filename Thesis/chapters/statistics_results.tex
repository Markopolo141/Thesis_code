...
\end{itemize}
We consider that the three methods (\textsc{Ney},\textsc{Ney-W} and SEBM*) are built apon the assumption of known variances for the strata, which are supplied to them, so that they may serve as a comparrison with the performance which would be possible for various methods which such information.
Additionally we note that for all other methods (where appropriate) we selected for minimising a 50\% confidence interval (i.e. constant $p=0.5$ and $t=0.5$).

The differences between these methods provide comparisons of different algorithmic factors, such as the dynamics of sampling: with and without replacement; with stratification and without; between our method and Neyman sampling, and; with and without perfect knowledge of stratum variances. 
...
with $\alpha$ and $\beta$ parameters drawn uniformly between 0 and 4 for each stratum, and $\Gamma$ is the gamma function.
