...
\item \textsc{SECM-W} (Stratified empirical Chernoff method): the method of iteratively choosing samples \textit{without} replacement from strata to minimize Equation~\eqref{another_big_equation}.
\end{itemize}
Note that the last three methods (\textsc{Ney},\textsc{Ney-W} and SEBM*) assume and utilize prior perfect knowledge of the variance of each of the strata, and that for methods SEBM, SEBM-W, SEBM* and SECM (which use Equations~\eqref{big_equation}, \eqref{eq1} and \eqref{another_big_equation}) we selected for minimising a 50\% confidence interval (i.e. constant $p=0.5$ and $t=0.5$).
