...
Particularly, we consider the possible sampling processes to approximate the exponentially many terms inherant to the GNK value (particularly the summation over all possible coalitions in equation \ref{da_value_eq}).
Secondly, we also consider a proxy inplace of the minimax optimisations at the heart of the GNK value (per equation \ref{knvalue1}).

Using these two remedies it is seen that it is easily possible to calculate the GNK value on networks upto size of about $100$ players within $10$ minutes of runtime on a desktop PC, and we take an example network of this size and calculate the GNK value against LMP and VCG.
We then compare the features of GNK against LMP and VCG, before making a more thorough discussion of the GNK value against desirable ethical criteria identified in chapter \ref{sec:philosophy}.
...
To compute the GNK value to a required accuracy, not all of the minimax optimisations $v(S)$ need to be performed (per equation \ref{knvalue1}), as sampling techniques may be used.
...
In this section we consider and discuss some of the behaviour exhibited by the M-GNK value against VCG and LMP for randomly generated larger networks.
