...
In the next Section \ref{} we will consider probability bounds specifically developed for sampling without replacement. However it is sufficient to note that all these probability bounds also hold in the case of sampling without replacement; but the converse is not nessisarily true. 
...
Using this knowledge, stronger bounds are very possible - and we explore and develop this in the following Section \ref{}.
...
In this Section we develop a novel EBB derived from numerical computations, and compare it as a stratified sample mechanism against stratified sampling done with Maurer and Pontil's EBB and Audibert et.al's EBB, and also Neyman sampling in Chapter \ref{}.
...
The reason for the separation of Bennet's inequality into two parts is that we will reuse the Lemma to derive a weaker but more mathematically manipulatable inequality later (as theorem \ref{}).
...
Additionally the derivation of Hoeffdings inequality (per Theorem \ref{Hoeffdings_inequality_proper}) is built with a mathematically simplifying assumption that is not nessisary - but can be conducted without it as Theorem \ref{} in the Appendix \ref{}.
...
\caption{$g(x)=(e^{-qd^2}-1)d^{-2}x^2+1$ over function $f(x)=e^{-qx^2}$ for all $a\le x\le b$ where $d=\max(b,-a)$}%
\label{fig:graph111}%
\end{minipage}%
\end{figure}
...
Though this parameter sweep on $b$ may not necessarily respecting analytical accuracy (since it is a maximisation, not a minimisation), we convert to this form only for the purposes of broad comparison with other bounds on the variance.
...
From this graph we see that our bound performs better at creating lighter tail bounds for the sample variance estimator but that different methods work better-or-worse in different regions - as divergent methods produce differing results.
...
In this process care must be taken in applying this theorem that all the assumptions hold, the necessary inverses exist, and the domains of the functions are propagated through the analysis.
...
By being able to numerically evaluate our EBB, we were abel to compare the strength of our results against existing concentration bounds, particularly our EBB is compared to Maurer and Pontil's EBB and, Bennett's inequality with perfect variance information.
...
But that generally, we observed that our refinement of M\&P's EBB was expectedly uniformly tighter across a large range of values.
