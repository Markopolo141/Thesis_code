...
particularly, the leveling down objection is this: that an egalitarian distribution is not nessisarily pareto optimal.
...
Additionally there are more egalitarian approaches that allocates the equal utility outcome that is secondarily maximising.
...
%One particular and common way of associating quantities to preferences (in practice) is in term of money. particularly that the value of a particular outcome might be associated with how much money a person might be willing to part with (or be given) in order to be indifferent to its occurance over another.
%However money is not the only measure, other example measures where the desirability of an outcome are associated with measures include the profitability of a venture, the average number of stars of a movie's reviewers, a governmental policy's affect on GDP, or the user satisfaction ratings of a service, etc.
