...
So for instance, the concept of Equality can be inclusive-of or contrasted-against other views, \footnote{including ones that can address the levelling-down objection: that a strict philosophical Egalitarian would prefer a world in which everybody was worse off if it were equal, see \cite{temkin_2003, equalityandpriorityparfit}}; for instance, such as thoes that emphasize the priority of resources to the poor, or such as emphasise alleviation of insufficiency among the poor; philosophies sometimes broadly termed ``prioritarianism'' and ``sufficientarianism'' respectively.\cite{sep-egalitarianism}%\footnote{for good measure we might also consider Rawl's Theory of Justice \cite{rawls2005theory} as a specific kind of embedded priority principle}
...
Particularly famous devices include Rawl's ``Original Position'', Hare's "ideal sympathiser", Kant's Universality categorical imperative, etc.
...
This is known as formal equality \cite{whatisbasicequalitynathan} and is occasionally seen as being an important part of a fair system, but also insufficient to capture broader notions of equality and justice.
...
And indeed, by imagination almost any treatment or process could be rationalised as being by principles which are universally applied.
Hence our focus  turns to the more vague notion of inequalities between different kinds of rules (and treatement) that people can be effectively subject to \textit{in practice}.
