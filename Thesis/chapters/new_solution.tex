...
But if we directly extend the 2-player process to 3 or more players, then there isnt a distinctly defined or unique nash equilibrium in a game of 3+ players.
And so having a nash disagreement point becomes ambiguous.

While it may well be possible to define one of thoes possible nash equilibria in a 3+ person game as being the disagreement point. or perhaps privilage some other measure to that position.
However our recourse is to instead consider all the possible divisions of players between two groups and then consider all the 2-player nash equilibrium points between them. And then to integrate this information via the shapley value to form a unique outcome.

This was the process discovered by Harsanyi in 195X. and we extend it here.

So far as we know Harsanyi's solution concept of this kind has never been applied to electricity networks, particularly as there is a problem in doing so.
That in electricity networks the mutual actions of participants are limited, not everybody can do any combination of specific actions, (such as might overload power lines etc)
And so this limitation on mutual actions forms a generalized game, and unfortunately in the context of a generalized game there is no necessarily unique minimax point between two players.
And a remedy is needed in order to make the application.

The remedy that is proposed is to take the expected outcome on a coin-flip on who chooses actions in the minimax strategies - as we shall see. this -much more directly- is unique minimax outcome, that satisfies the application and leads to a coherent outcome.

The resultad solution concept we call the \textit{Generalized Neymann and Kohlberg Value} or the \textit{GNK value} for short.
as Harsanyi's solution was in-part axiomatically derived by Neynamm and Kohlberg.
And it is a solution concept for TU normal form generalized non-cooperative games.
Which is straightforwardly extensible to many contexts, but particularly we focus on OPF.
...
We then define the \emph{threat} or \emph{advantage} of a coalition $v(S)$ in the context of a \textit{generalized non-cooperative game} (which is a key point of novelty in our solution concept) which defines the GNK value.
...
This mapping gives a distribution of the total surplus $v(N)$ among the players, and N\&K appropriately call this unique mapping the `Shapley Value' of the game of threats \cite{KOHLBERG2018139} as it mirrors the classic \textit{Shapley value} of cooperative game theory~\cite{Shapley1953a}.
...
The players execute the strategies that achieve this maximal sum, and then the outcome of $\varphi$ can be implemented by executing the required utility transfers.
...
is the Value, which cannot be directly applied to generalized games because the condition $v_o(S)=-v_o(N\setminus S)$ can fail to hold in that case. 

The Value (and GNK value) is also conceptually related to Harsanyi's solution in this context \cite{values3},
