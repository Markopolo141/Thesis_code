...
In considering the social and ethical value of the GNK-type solutions we must loop back to cosider some of the topics explored in chapter \ref{chap:background}.
Particularly we briefly considered some of the moral elements which may be present in peoples thinking about the distribution of resources, including Equality, Efficiency, and the various normative reference points which may be considered.
We will consider the GNK in light of each of these in turn.

\subsubsection*{Efficiency}
It is easy to consider that the GNK value is axiomatically efficient, specifically in terms of maximising the sum of utility, by axiom (equation \ref{myeq}).
...
Additionally, it is also quite easy to note that because the GNK is efficient in this regard it is also therefore pareto optimal.
...
The implementation of the GNK value assumes that utility is transferrable, and the easiest way of considering this is in terms of utility as measured by money; in this context the maximisation of utility corresponds to the maximisation of the monetary value of outcomes.
...
As it is developed in this text, the GNK value is inherantly utilitarian in the most classical sense, but similar solution concepts could be made to bias it towards other efficiency measures.

\subsubsection*{Formal-Equality}

All the solutions explored in this text satisfy variations of formal-equality, as there is no mechanism specifies distinct behavior or treatment for any individual participant.

In all the techniques (GNK, VCG and LMP) only treat different participants differently in relation to their utility, and how their presence can affect the utility of others.
...
So, for instance, for some people, the idea of people being afforded different prices for their electricity depending on their location may be regarded as being ethically unfair, however this would also negate any possible incentive to install generation capacity in advantageous locations.
...
Particularly it inherits the underlying idea that the disagreement point in the non-cooperative game between two parties is the normative reference point from which the dynamics of the game are manifest.
Particularly it assumes that the minimax of the zero-sum game is a point which is/should-be important to the dynamics of the system.
It raises the question - should it be?
particularly the minimax of the zero-sum game identifies a point of maximum strategising to gain advantage specifically over the opponent, irrespective of the gain ultimately given to the player.
In this way it identifies a point of utter maximum competition between interest groups.
the GNK value builds apon this to consider the space of all possible coalitions and anti-coalitions which could form, and amalgamates all possible minimax's between them into a point of value for each participant in thoes formations.

In some way, it assigns equal weight for every possible coalition that could form, irrespective of the likilihood of that materiality.
Additionally, it does not assume that groups of players might be altruistic between themself (as represented in their utility function) but does assume that some degree of perfect competition - is this realistic basis for a normative reference point, should resources be distributed above the point of perfect competition?

For instance, GNK contrasts against other similar solutions over non-cooperative games (as briefly mentioned in section \ref{relating_to_the_old}) where von Mortgenstein solution, where the characteristic function is identified as minimax payoff, not payoff advantage - would this be a more realistic normative reference point? is difficult to tell.

Furthermore the GNK contrasts against LMP and VCG, particularly in regards to the reference point which they build apon.
particularly the VCG notion builds apon the idea that the value of a player should be in reference to their 'absense' only at the solution point of the grand coalition, and therefore cannot allocate resources to thoes who do not interact with such a solution point; and a similar dynamic is present in LMP, in terms of marginal interaction.

between these solutions there are different choices of reference point from which the dynamic occurs, an the question which is pertinent (and still unanswered) is what reference point should be chosen.

One reference point, which is baked-in to VCG is the reference point of zero, the axiom of individual rationality casts the dynamics such that each participant will be better of than if they didnt participate in the mechanism.
this Important reference point is not present in GNK (as it is derived) and we discuss it in the next point.

\subsubsection*{Wider Equality}

GNK seems to violate an individual rationality constraint, in that it is possible for participants to walk away with less than they started with, leaving them regretting partaking in the system. when designing GNK for electricity systems, it was hoped that invidiual rationality would `fall out', and in the neyman-kohlberg formulation it might well fall out, in that the most any agent can threaten to do is to not particiapte in the system, whereas under GNK value, the power conservation constraint allows participants to extortionately threaten to oversupply others, leaving them worse off then when they were before. this is a major ethical failing.

The Neyman and Kohlberg value (if it is possible for every partipant to unilatteraly implement a strategy that yeilds them zero) may well be individually rational.
