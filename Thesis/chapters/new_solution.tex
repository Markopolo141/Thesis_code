...
The primary motivation for this new solution, is to attempt to distill the intuition behind the Nash bargaining solution concepts (per section \ref{sec:solutions_bargaining}) into a description of total allocation that is suitable for arbitrary number of players - rather than just two.
...
We begin by presenting the axiomatic foundations of the GNK value, in a similar manner as Neyman and Kohlberg's exposition \cite{value2}.
...
We can eliminate redundant variables, such as $\theta_i$ and $p_{i,j}$, and to ease presentation, and use the abstract functions $h_j$ and $g_k$ (for indeices $j,k$) to represent the remaining linear functions:
...
The essential feature of the GNK value is that it is an extension of bargaining solution concepts to multiple players over the restricted strategy space of a generalised non-cooperative game.
