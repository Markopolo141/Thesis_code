...
Neither the GNK value or LMP are \emph{incentive compatible} in the sense that is often referred-to in mechanism design, but that VCG is.
...
Misreporting these utilities may change the $v(S)$ and hence the GNK value itself.
...
This can be seen via \eqref{da_value_eq} where calculating the GNK value exactly requires calculating $v(S)$ for all the $2^n-1$ possible coalitions $S$, and each calculation of $v(s)$ are potentially NP-hard bilevel problems.
...
Since computing the GNK value is difficult we review two primary remedies to approximate the GNK value for larger and more realistic networks:

\begin{itemize}
    \item We employ sampling techniques to reduce the number of minimax optimizations that need to be conducted to approximate the GNK value to sufficient accuracy.
...
To compute the GNK value to a required accuracy, not all of the $v(S)$ for all of the $2^n-1$ coalitions $S$ of players need to be performed, as sampling techniques may be used.
We consider two different approaches for bias free sampling of the GNK value:
\begin{enumerate}
    \item By the inspection of equation \ref{da_value_eq}, we see that the GNK value of any player $i$ is an average over $v_{i,k}$ (the average value $v(S)$ for coalitions $S$ of size $k$ that include player $i$), and that by choosing random coalitions of size $k$ which include $i$ we can sample for an estimations of each $v_{i,k}$.
...
On this graph it is seen that the methods which utilized stratification (\textsc{Hoeffding}, \textsc{Neyman}, \textsc{SEBM} and \textsc{Join}) were given a warm-start of 200 samples (as methods like \textsc{Neyman} required atleast two samples per stratum minimum for a bias-free estimate of the variance) and generally performed better than those that did not use stratification (particularly \textsc{Simple} and \textsc{Appro}).
In this graph, is show the number of unique optimisations $v(S)$ called during the sampling process, and since these are 10 bus network, there is only 1024 unique optimsations that can be called; as these methods sample with replacement it begins to be time-prohibitive to sample to perfect accuracy, and so they were apprehended beforehand.
Each of these methods were carried out by sampling with replacement primarily as in the context of large networks (where there are $2^n-1$ possible coalitions) generating and finding unique coalitions proves itself to be a bottleneck challenge - and so sampling without replacement becomes impractical; and so we carried out the testing with sampling with replacement.

\subsection{Determination of Sampling Method}
...
The reason for this superiority is the additional power of solving across join orders over choosing coalitions randomly.
As allready stated, by using join orders, it is possible to calculate $n$ marginal contributions using $n+1$ evaluations of $v(S)$ as opposed to the random selection of coalitional contributions, which thus taking two evaluations of $v(S)$ for one marginal contribution sample point.
This simple factor outweighed all the extra computing power associated with \textsc{SEBM} and \textsc{Neyman} methods.
...
And this was somewhat expected, as the GNK value is a calculation that involves exponentially many NP-hard computations to determine.
