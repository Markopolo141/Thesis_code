...
One primary question is whether the behavior exhibited in figure \ref{fig:experiment_table_13} is entirely the result of using a modified characteristic function (equation \ref{knvalue22}) over the more original characteristic function (equation \ref{knvalue1}).
...
From this graph it is noticed that the GNK and modified GNK values feature similarity which seems to increase with the size of the network under consideration --- although it is increasingly difficult to confirm for networks with a size greater than 13.
...
Another observation, is that while the GNK remains soluble for networks of less than 30 nodes and the modified-GNK value remains soluble for upto 80 node networks, theese might be regarded as being too small for real-world electrical network modelling.
Though slightly larger networks are expected to be tractable for modified-GNK value with more computing power and/or execution time, further methodological improvements may ultimately be necessary to bring the GNK value (or anything like it) to be capable of bearing on real world sizable networks.
Some possibilities include employing further approximations such as player clustering \cite{DBLP:journals/corr/abs-1903-10965}, or transforming the problem into non-atomic form of Shapley Value structure.
...
Another consideration, is that of contemplating the likely outcome of strategising by the network participants.
While VCG has explicit consideration of the strategising of the agents, and LMP is identified to be largely identical to it in the context of large nentworks, what is the likely outcome of agents strategising under GNK?
Particularly the GNK is incentive incompatable, and the consequences of agent strategising under it are unknown and our research has not yet addressed the potential consequences of this.
While there does exist some work on similar bargaining solution concepts like GNK that are incentive compatible \cite{myerson1,Salamanca2019} their investigation and evaluation in the context of electricity networks remain a topic for further investigation.
...
In considering the social value of the GNK-type solutions we must loop back to cosider some of the topics explored in chapter \ref{chap:background}.
...
The most straightforward and easy notion to address is that the GNK value is axiomatically efficient, in terms of maximising the sum of utility - and is therefore pareto optimal, it allocates electrical outcome which maximises the sum of utility, and instructs budget-balanced utility transfers between the players, neither LMP or VCG has a similar claim.

The second most straightforward observation is that all the solutions explored in this thesis satisfy variations of formal-equality, particularly as there is no part of any mechanism (GNK, VCG, LMP) that specifies distinct behavior or treatment for any individual participant barring the characteristics of location and electrical capacity (to consume/generate) and the utility associated with these.
These satisfy formal equality of a certain sense, and rais the question of whether or not these characteristics are morally warranted bases for differential treatment.
For some the idea of people being afforded effectively different prices for electricity depending on their location or non-linearly depending on their generation capacity may be regarded as being unfair.
These and similar points are the subject of wider discussion, and the motivation to explore how it could not be so unfair is the motivation for this research addressed in section \ref{sec:NEM_background}.

The next most important observation is that all the different solution concepts relate to different normative reference points, particularly that VCG


Particulary, there is no bias towards thoes who are worse-off 
...
In conjunction with Chapter \ref{}, we see that althought the GNK value satisfies many ethical pricniples including hard efficiency, formal equality, and consistent reference point.
