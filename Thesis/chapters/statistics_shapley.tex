...
%Unfortunately we don't know $a$ and $b$ but know that the data is bounded within a width of $D=b-a$.
%Thus we subsequently take the worst case $a,b$ consistent with a given $D$ and also $\sigma^2\le-ab$ and by this process create a bound:
...
%From this graph we see that our bound performs better at creating lighter tail bounds for the sample variance estimator but that different methods work better-or-worse in different regions - as divergent methods produce differing results.\footnote{We do however note that the graph our our variance bound on this figure was produced after taking the worst case $a,b$ consistent with a $D=1$, and the manner in which we use it subsequently avoids this deterioration, until a later stage.}
