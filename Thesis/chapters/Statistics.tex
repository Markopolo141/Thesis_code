...
This process proceeded by binding EBBs together using union bounds to create a bound on the stratified sample mean error (via with Theorem \ref{triangle_theorem1}); which is then sequentially minimised by the choosing of additional samples.
...
Using this knowledge, stronger bounds are very possible - and in this section is created an empirical (ie. depending on sample variances) concentration inequality specifically for stratified random sampling; which then can be used to serve as a basis for choosing samples.
...
Specifically we consider three upper bounds for various moment generating functions.
...
The other two are strongly related to Theorems \ref{hoeffdings1} and \ref{sample_squares} respectively, except that they use parabolic upper upper bounds.
...
\subsection{The Stratified finite Empirical Bernstein Bound (SEBB) and sampling Method (SEBM)}
...
particularly since the last Theorem \ref{thm:SEBM_bound} ultimately builds upon Theorem \ref{thm:1} which was the embodiment of a simplification of Bennett's inequality per Lemma \ref{expectation1}.
And since we will compare against Neyman sampling which is conceptually built upon the minimisation of Chebyshev's inequality (Theorem \ref{thm:chebyshevs}) we will also consider and compare against a empirical Chebyshev's inequality for stratified sampling.
