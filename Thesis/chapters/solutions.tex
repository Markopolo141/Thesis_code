...
In the previous chapter \ref{} we discussed some of the philosophy of distributive ethics, and in this chapter we discuss the various ways thoes ideas can be made quantitatively precise.
...
All of them tacitly respect formal equality (per section \ref{sec:formal_equality}), with bargaining and cooperative game theory concepts satisfying absolute efficiency requirements (per section \ref{sec:philosophy_efficiency}), with LMP often doing so.

Each of the formulations presented has various background and unique properties and we will introduce and discuss each of them in turn, before we attempt a new synthesis in the next chapter \ref{cha:new_solution}.
...
This raises the question of when the Shapley value is in the Core? - and therefore has that particular stability that would disincentivise any subgroup from leaving.
...
\end{equation}
Which is a condition which might often be expected to hold each coalition unit is informally `greater than the sum of its parts'.

\subsection{Applications to Electricity Systems}

Shapley value concepts have often been considered as a potential mechanism for pricing in the various facets of electricity system operation.
...
There exist many other solution concepts in the context of cooperative game theory, such as the Kernel and Neucleolus. The interested reader is encouraged to read more \ref{RePEc:cup:cbooks:9781107691322}.

However in the next section \ref{sec:solutions_bargaining} we consider some solution concepts which apply in circumstances where it is not easy to describe the `worth' or `value' of a coalition of players; where strategic considerations between them are important and it is not easy to make sense of a characteristic function.
...
In the previous section \ref{}, we considered a way of describing normalised pricing in large markets, however in this section we consider some of the various ways of describing normalised pricing among a small number of market participants.
