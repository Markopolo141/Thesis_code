...
Discussing the philosophy of distributive ethics is important, as we considered in the previous chapter \ref{}.
However for each of these ideas there is a range of ways which they can be made quantitatively precise.
Particularly in this chapter we consider a range of ways that some of our moral ideas about distribution have been mathemetised.
We highlight the most famous and relevent mathemetisations, because our development extends from them, and also ultimately contrasts against them.
Particularly we consider the following:
\begin{itemize}
\item VCG,
...
In section \ref{sec:reference_points} we explored the idea that people might be rewarded in proportion to their contribution - particularly the value that they add above their absense.
And while this idea may be uncomplicated, its mathematization has some suprising features.
We consider the most direct mathemetisation of this idea as per the VCG mechanism with Clark pivot.

Particularly the VCG mechanism (with Clark pivot) is an allocation process where each player is payed money (or `transferable utility') equal to the impact their presense has on others (ie. their externality) in the decision process which selecting an outcome that maximises utility.
...
Then any agent $i$ has a valuation for any outcome in $X$:
...
This outcome is implemented and the bidding process pays each agent $i$ the value $p_i$:
...
This is the VCG mechanism with Clark pivot, which might be seen as a straightforward way of allocating ethical payments, and has some particular properties.
The first and most notable positive property is that if the utility of the agents is quasilinear, ie. that their utility is their valuation plus any transfer they recieve, and their utility has form:
$$ u_i = p_i(v)+v_i $$
Then VCG mechanisms like this are \textit{truthfull} or \textit{incentive compatable}, in that no player can possibly gain from misreporting their valuations.
\cite{roberts1979characterization, Lavi2008}
Additionally it has the property called \textit{individual rationality} in that no agent (assuming quasilinear utility) will left with a negative utility.
However one drawback of this mechanism is that it is not \textit{budget balanced}, in that it is possible for the amount that is payed between the players might not add to zero.
It is also \textit{efficient} in the sense that it actualises the utilitarian socially optimal solution $x^*(v)$.

Each of these properties is often held to be desirable. Truthfull mechanisms induce all agents to report truthfully and hence eliminates the need for any strategising by them, which might otherwise be a source of system instability.
Individually rational mechanisms is often viewed as a basic fairness property, in that nobody is left any worse-off for having participated in the system - which is a key factor in fairness as described by euvoluntary exchange (see section \ref{}).
And budget balance is another important feature, in that the process can be maintained without budgetary injection, and without taking net moneys from the participants.

Unfortunately all of these three properties are impossible to combine in the general case where valuations (or preferences) are unrestricted.(find a good source, of, myerson satterthwaite theorem)

The VCG mechanism is a cornerstone discovery in the field of Mechanism Design, and there exist many good sources giving extended discussion on the surrounding theory and the variations about the technique.\cite{37377}


One of the more important features of Mechanism design that the VCG mechanism illustrates is the potential, for considering the way in which any system that is implemented will behave between strategising agents.
and the result of that interraction leading to system dynamics.
Particularly that for any system implemented, it is possible to design a system which is truthfull and will yeild the same nash equilibrium as the system would fall into anyways - this is known as the revelation principle (CITE CITE CITE)

What we can do is to consider the way in which the truthfullness property of VCG coincides with other 
%TODO
