...
\section{The changing nature of supply, and the introduction of demand elasticity}\DIFaddbegin \label{sec:intro_part1}
\DIFaddend 
...
\DIFaddbegin \DIFadd{With the consideration of demand response programs, the question is how the time-limited curtailment of consumption of electricity be traded and valued?
}

\noindent \DIFadd{And this reflects the broader question: ``how should electricity be valued and traded?'' 
}
...
However, \DIFdelbegin \DIFdel{one limitation of }\DIFdelend \DIFaddbegin \DIFadd{a limitation of some }\DIFaddend traditional generating technologies \DIFaddbegin \DIFadd{that operate with large synchronous rotors, }\DIFaddend is that they are \DIFdelbegin \DIFdel{often }\DIFdelend unable to respond quickly to assist in the maintaining of system frequency, and therefore unable to support grid frequency or bid into FCAS markets \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{due to their ramp-rate limitations and costs. \mbox{%DIFAUXCMD
\citep{GONZALEZSALAZAR20181497}
}\hspace{0pt}%DIFAUXCMD
}\DIFaddend And while there does exist some technology for rapid response gas generators (as discussed by \cite{GONZALEZSALAZAR20181497}) the more promising renewable technology for this purpose is battery technologies, which have characteristically fast response times.
...
%DIFDELCMD < %%%
%DIF < Another avenue currently being ivestigated is contracting strategic reserve power for grid stability. which are currently being formalised into the Reliability and Emergency Reserve Trader (RERT) rules.
%DIFDELCMD < 
...
\DIFaddbegin \DIFadd{Another avenue currently being ivestigated is the contracting strategic reserve power specifically for grid stability. which are currently being formalised into the Reliability and Emergency Reserve Trader (RERT) rules.
}

\DIFadd{With the consideration of the various ways of trading, valuing and integrating storage in providing diachronic arbitrage of electrical energy to offset frequency deviations, the question is how the storage and time-sensitive rapid injection/consumption of electricity be traded and valued? Because an answer to this should be a consequence of an answer to the broader and more general question: ``how should electricity be valued and traded?''
}\DIFaddend 
...
However, there may be future role for electricity consumers \DIFdelbegin \DIFdel{(or so called `prosumers') }\DIFdelend not only to participate in network support and have access to market structures; but also ideas that might induce consumers to create more social benefits, such as directly sharing their electricity with each other.
...
There are many different visions of what the structure the future electricity system might have, and what features it might have to support trading between \DIFdelbegin \DIFdel{prosumers}\DIFdelend \DIFaddbegin \DIFadd{producer-consumers, or so-called `prosumers'}\DIFaddend , such as \DIFdelbegin \DIFdel{sketched }\DIFdelend \DIFaddbegin \DIFadd{considered }\DIFaddend out by \cite{Parag2016}.
These future visions describe ideas about how prosumers might trade directly or indirectly between themself and the wider grid; such as perhaps between P2P trading and/or aggregation into many larger virtual power plants (VPPs) or via localised community storage\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Morstyn2018}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{. \mbox{%DIFAUXCMD
\citep{Morstyn2018}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
...
\DIFdelbegin \DIFdel{The continued proliferation of DERs is a technological change that is expected to create investigation into these future visions of the grid}\DIFdelend %DIF > The continued proliferation of DERs is a technological change that is expected to create investigation into these future visions of the grid.
\DIFaddbegin 

\DIFadd{With the consideration of the various ways of facilitating the fair trading of energy between heterogeneous devices on distribution networks between consumers, the relevant question is how to integrate these various factors in a flexible and equitable way}\DIFaddend .
...
%DIF < peer-to-peer electricity trading and managment on small-scale networks - sometimes called microgrids - have been developed in various contexts around the world, including Australia
%DIF > peer-to-peer electricity trading and management on small-scale networks - sometimes called microgrids - have been developed in various contexts around the world, including Australia
...
\DIFdelbegin \DIFdel{"}\DIFdelend \DIFaddbegin \begin{displayquote}
\DIFadd{``...}\DIFaddend all competitors, irrespective of their size or financial strength, get equal opportunity to compete. It is not enough if all players play by the same rules. The rules must accommodate the needs of all, whether small or large, so the market is free of impediments to smaller players.\DIFdelbegin \DIFdel{" \mbox{%DIFAUXCMD
\cite{australianenergymarketoperatorlimited2018}
}\hspace{0pt}%DIFAUXCMD
...
Ethical criteria such as this, as well as wider political and social implications \DIFdelbegin \DIFdel{bring philosophy and ethics into our research question}\DIFdelend \DIFaddbegin \DIFadd{bear on this discussion}\DIFaddend . 
Thus our investigation is to explore and evaluate the research question of what \DIFdelbegin \DIFdel{market structures }\textit{\DIFdel{should}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{kinds of market structures should }\DIFaddend be implemented in \DIFdelbegin \DIFdel{this new world}\DIFdelend \DIFaddbegin \DIFadd{the future}\DIFaddend .
...
\DIFaddbegin \noindent \DIFadd{By these considerations we can attempt a new answer to the broader and more general question: ``how should electricity be valued and traded?''
}


\DIFaddend \section{Research and problem approach}

The fundamental \DIFdelbegin \DIFdel{question about what `market structures }\DIFdelend \DIFaddbegin \DIFadd{research question is `how }\DIFaddend \textit{should} \DIFdelbegin \DIFdel{be implemented'}\DIFdelend \DIFaddbegin \DIFadd{electricity be valued and traded?', which is general and multifaceted question without an easy or established answer (from previous sections \ref{sec:intro_part1}-\ref{sec:intro_summary})
}

\DIFadd{The research question }\DIFaddend is easily identified as having a moral and ethical quality, and thus in Chapter \ref{sec:philosophy} we survey some of the relevant philosophy associated with the ethics of \textit{Distributive Justice}.
...
The SEBM was derived as a online method of choosing samples in the context of stratified sampling, where the orthodox method of choosing samples (called Neyman sampling) \DIFdelbegin \DIFdel{takes a necessary }\DIFdelend \DIFaddbegin \DIFadd{necessarily takes }\DIFaddend two unique stages to complete.
...
\DIFdelbegin \DIFdel{Additionally each }\DIFdelend \DIFaddbegin \DIFadd{All }\DIFaddend of the stratified sampling methods were identified as \DIFdelbegin \DIFdel{begin }\DIFdelend \DIFaddbegin \DIFadd{being }\DIFaddend applicable for sampling the Shapley Value and GNK value, and the performance \DIFdelbegin \DIFdel{at these tasks were }\DIFdelend \DIFaddbegin \DIFadd{of these sampling methods for such a task was }\DIFaddend evaluated.
...
Within \DIFdelbegin \DIFdel{our research our primary contributions }\DIFdelend \DIFaddbegin \DIFadd{the research program, the primary contributions made }\DIFaddend are:
\begin{itemize}
...
\noindent``The Generalized N\&K Value: An Axiomatic Mechanism for Electricity Trading'' \DIFaddbegin \DIFadd{by Mark Burgess, Archie Chapman and Paul Scott}\DIFaddend \\ International Conference on Autonomous Agents and Multiagent Systems\\ (AAMAS) 2018\\
(accessible: \DIFdelbegin %DIFDELCMD < \href{ifaamas.org/Proceedings/aamas2018/pdfs/p1883.pdf}{ifaamas.org/Proceedings/aamas2018/pdfs/p1883.pdf}%%%
...
\noindent``An Engineered Empirical Bernstein Bound'' \DIFdelbegin \DIFdel{,}\DIFdelend \DIFaddbegin \DIFadd{by Mark Burgess, Archie Chapman and Paul Scott}\DIFaddend \\ European Conference on Machine Learning (ECML-PKDD) 2019\\
(accessible: \DIFdelbegin %DIFDELCMD < \href{http://ecmlpkdd2019.org/downloads/paper/435.pdf}{ecmlpkdd2019.org/downloads/paper/435.pdf}%%%
...
\noindent``\DIFdelbegin \DIFdel{Stratified Finite }\DIFdelend \DIFaddbegin \DIFadd{Approximating the Shapley Value Using Stratified }\DIFaddend Empirical Bernstein Sampling'' \DIFdelbegin \DIFdel{, Preprints 2019}\DIFdelend \DIFaddbegin \DIFadd{by Mark Burgess and Archie Chapman}\\ \DIFadd{International Joint Conference on Artificial Intelligence (IJCAI-2021)}\DIFaddend \\
(accessible: \DIFdelbegin %DIFDELCMD < \href{preprints.org/manuscript/201901.0202/v2}{preprints.org/manuscript/201901.0202/v2}%%%
\DIFdel{)}\DIFdelend \DIFaddbegin \href{https://www.ijcai.org/proceedings/2021/0011.pdf}{https://www.ijcai.org/proceedings/2021/0011.pdf}\DIFadd{)}\\


\DIFadd{Particularly, we would like to make note some specific contributions: that Archie Chapman provided the idea of using Empirical Bernstein Bounds to derive novel method of sampling the Shapley Value.
...
\begin{enumerate}
\item in Chapter \ref{sec:philosophy}, we give a series of brief philosophical points to provide ethical background for the underlying question of `how \textit{should} electrical energy be traded?'. In this section, we refer to the diversity of conceptions about social Equality, the different ways in which systems can be considered better/worse apart from equality (particularly by notions of Efficiency), and by ethical rules and guidance in proportion to various norms and reference points\DIFaddbegin \DIFadd{, introducing notions of envy-freeness, in a broader environmental context}\DIFaddend . 
...
\item in Chapter \ref{cha:new_solution}, we develop and explore a new solution concept called the GNK value, that is derived from Shapley Value axioms and relates directly to Nash bargaining, and we compare it against LMP and VCG results in the context of \DIFdelbegin \DIFdel{simulated electricity networks. Particularly the }\DIFdelend \DIFaddbegin \DIFadd{a small-scale simulated electricity network, we make observations about its qualities point-by-point and consider the difficulty in computing it for larger networks.
}\item \DIFadd{in Chapter \ref{sec:scaling} We address the difficulty in computing the GNK value by exploring two approaches to approximate it, particularly the use of sampling schemes, and the use of a proxy for the inner terms of the GNK value. The }\DIFaddend advantages and disadvantages of the new approach are discussed \DIFdelbegin \DIFdel{and related back to the ethical desirata established in Chapter \ref{cha:background}.
The particular difficulty of computing this new }\DIFdelend \DIFaddbegin \DIFadd{in relation to its computed result on a larger electricity network.
The }\DIFaddend GNK value is \DIFdelbegin \DIFdel{addressed and overcome by utilising sampling techniques in the presence of a proxy}\DIFdelend \DIFaddbegin \DIFadd{discussed with regard to the ethical qualities established in Chapter \ref{cha:background}}\DIFaddend .
\item in Chapter \ref{chap:stratified_sampling_chapter}, we investigate a range of stratified sampling techniques which were developed for computing the new GNK value. this investigation covered different techniques of conducting stratified sampling by minimising concentration inequalities, and a new technique was resolved called the stratified empirical Bernstein method (SEBM)\DIFaddbegin \DIFadd{.
}\item \DIFadd{in Chapter \ref{sec:the_thesis_conclusion} we conclude the thesis by providing reflections of the approach against the situations highlighted in this chapter, as well as outline future work and summarising major accomplishments}\DIFaddend .
\end{enumerate}
