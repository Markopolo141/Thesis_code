...
The minimisation of existing EBBs (such as Theorems \ref{MandPsEBB} and \ref{AudibertsEBB}) cannot be directly be used as a method of choosing samples in stratified sampling\DIFdelbegin \DIFdel{.
Since the aggregated }\DIFdelend \DIFaddbegin \DIFadd{, since the }\DIFaddend population mean estimate does not \DIFdelbegin \DIFdel{itself }\DIFdelend have a sample variance \DIFdelbegin \DIFdel{, }\DIFdelend but only the strata have \DIFdelbegin \DIFdel{means and }\DIFdelend sample variances. Such EBBs can be applied as a bound for the mean of an individual stratum \DIFdelbegin \DIFdel{, but such an EBB }\DIFdelend \DIFaddbegin \DIFadd{but }\DIFaddend cannot bind the aggregation of the mean estimates of all the strata.
...
\DIFaddbegin \DIFadd{We derive }\DIFaddend the following two theorems \ref{triangle_theorem1} and \ref{triangle_theorem2} \DIFaddbegin \DIFadd{that }\DIFaddend give derivations on the error, and the absolute error, of the stratified mean estimator.
...
Then the error in our stratified estimation \DIFaddbegin \DIFadd{of the population mean itself }\DIFaddend is probability bounded:
$$ \p\left(\hat{\mu}-\mu \ge \sum_{i=1}^m\frac{N_i}{\sum_kN_k} Z(n_i,D_i,\hat{\sigma}^2_i,t/m)\right)\le t $$
\end{theorem}

\input{exerpts/old_statistics_equation.tex}
...
%DIF < \begin{equation}\label{equation_partial_union2} \p\left(\sum_{i=1}^m\frac{N_i}{\sum_kN_k}(\hat{\mu}_i-\mu_i) \ge \frac{N_i}{\sum_kN_k}Z(n_k,D_k,\hat{\sigma}^2_k,t) + \sum_{\substack{i=1 \\ i\ne j}}^m\frac{N_i}{\sum_kN_k}(\hat{\mu}_i-\mu_i)\right) \le t \end{equation}
%DIF > \begin{equation}\label{equation_partial_union2} \p\left(\sum_{i=1}^m\frac{N_i}{\sum_kN_k}(\hat{\mu}_i-\mu_i) \ge \frac{N_j}{\sum_kN_k}Z(n_j,D_j,\hat{\sigma}^2_j,t) + \sum_{\substack{i=1 \\ i\ne j}}^m\frac{N_i}{\sum_kN_k}(\hat{\mu}_i-\mu_i)\right) \le t \end{equation}
...
%DIF < $$ \p\left(\hat{\mu}-\mu \ge \begin{matrix*} \frac{N_i}{\sum_kN_k}Z(n_k,D_k,\hat{\sigma}^2_k,t) +\\ \frac{N_l}{\sum_kN_k}Z(n_l,D_l,\hat{\sigma}^2_l,t) +\\ \sum_{\substack{i=1 \\ i\ne j\\i\ne l}}^m\frac{N_i}{\sum_kN_k}(\hat{\mu}_i-\mu_i)\end{matrix*}\right) \le 2t $$
%DIF < Repeating this process of taking equation \ref{equation_partial_union1} for a new index, adding appropriate terms to the inner inequality and using probability union lemma \ref{prob_union}, ultimately gives:
%DIF > $$ \p\left(\hat{\mu}-\mu \ge \begin{matrix*} \frac{N_j}{\sum_kN_k}Z(n_j,D_j,\hat{\sigma}^2_j,t) +\\ \frac{N_l}{\sum_kN_k}Z(n_l,D_l,\hat{\sigma}^2_l,t) +\\ \sum_{\substack{i=1 \\ i\ne j\\i\ne l}}^m\frac{N_i}{\sum_kN_k}(\hat{\mu}_i-\mu_i)\end{matrix*}\right) \le 2t $$
%DIF > We can repeat this process of taking equation \ref{equation_partial_union1} for a new index $q\ne j\ne l$, adding appropriate terms to the inner inequality and using probability union lemma \ref{prob_union}, gives:
%DIF > $$ \p\left(\hat{\mu}-\mu \ge \begin{matrix*} \frac{N_q}{\sum_kN_k}Z(n_q,D_q,\hat{\sigma}^2_q,t) +\\\frac{N_j}{\sum_kN_k}Z(n_j,D_j,\hat{\sigma}^2_j,t) +\\ \frac{N_l}{\sum_kN_k}Z(n_l,D_l,\hat{\sigma}^2_l,t) +\\ \sum_{\substack{i=1 \\ i\ne j\\i\ne l\\i\ne q}}^m\frac{N_i}{\sum_kN_k}(\hat{\mu}_i-\mu_i)\end{matrix*}\right) \le 3t $$
%DIF > and so on, and ultimately gives:
%$$ \p\left(\hat{\mu}-\mu \ge \sum_{i=1}^m\frac{N_i}{\sum_kN_k} Z(n_i,D_i,\hat{\sigma}^2_i,t)\right)\le mt $$
%And scaling $t$ gives result.
%\end{proof}

The result of this \DIFaddbegin \DIFadd{novel }\DIFaddend proof is an inequality bounding the error of the stratified mean estimate by the number of samples and the width and sample variance of each of the strata.
If instead of the error, we are concerned about the absolute error of the stratified estimate $|\hat{\mu}-\mu|$ then by a similar procedure (\DIFdelbegin \DIFdel{utilising }\DIFdelend \DIFaddbegin \DIFadd{inspired by the utilisation of }\DIFaddend the triangle inequality \DIFdelbegin \DIFdel{)}\footnote{\DIFdel{inspired by \mbox{%DIFAUXCMD
\cite{2013arXiv1306.4265M}}\hspace{0pt}%DIFAUXCMD
}} %DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{in the proofs of \mbox{%DIFAUXCMD
\cite{2013arXiv1306.4265M}}\hspace{0pt}%DIFAUXCMD
) }\DIFaddend we can derive a similar probability bound\DIFdelbegin \DIFdel{.
...
This method is made clear in pseudocode in \DIFaddbegin \DIFadd{technically-novel }\DIFaddend algorithm \ref{alg3}.
...
\label{alg3}
\begin{algorithmic}[1]
    \REQUIRE probability $t$, number of strata $N$, initial sample numbers $m_i$, initial stratum sample variances \DIFdelbegin \DIFdel{$\doublehat{\sigma}_i^2$}\DIFdelend \DIFaddbegin \DIFadd{$\hat{\sigma}_i^2$}\DIFaddend , widths $D_i$, maximum sample budget $B$,
    ...
    \WHILE{$\sum_k{m_k}<B$}
    	...
    	\STATE recalculate \DIFdelbegin \DIFdel{$\doublehat{\sigma}_{beststrata}^2$
    }\DIFdelend \DIFaddbegin \DIFadd{$\hat{\sigma}_{beststrata}^2$
    }\DIFaddend \ENDWHILE
\end{algorithmic}
\end{algorithm}
...
There are various EBBs which place variance-sensitive bounds on the mean, and it remains an outstanding task is to see how much these EBBs can be \DIFdelbegin \DIFdel{sharpened}\DIFdelend \DIFaddbegin \DIFadd{improved}\DIFaddend ; we take inspiration from the work of \cite{Maurer50empiricalbernstein} to develop a new and stronger EBB.
...
We \DIFdelbegin \DIFdel{state }\DIFdelend \DIFaddbegin \DIFadd{derive and present a refactored summary proof of Bennett's inequality as }\DIFaddend the theorem \ref{hoeffdings1}\DIFaddbegin \DIFadd{, }\DIFaddend whose proof involves the use of an intermediate theorem \ref{thm:parabola}.
\DIFaddbegin \DIFadd{We present it here in a way conducive to the manipulation we will subsequently perform with it (modifying it into proof of Theorem \ref{expectation1}).
}\DIFaddend 
...
The following \DIFaddbegin \DIFadd{novel }\DIFaddend concentration inequality was derived\DIFdelbegin \DIFdel{(which we note, appears to be novel)}\DIFdelend :
...
By Theorem \ref{hoeffdings1} and Lemma \ref{sample_squares} we have a probability bound on the mean squared and a probability bound on the sample squares\DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{, and from these }\DIFaddend it is possible to create \DIFaddbegin \DIFadd{novel }\DIFaddend a bound on the sample variance using lemma \ref{variance1}, as follows\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{:
}\DIFaddend 
...
To combine these two theorems to create a bound for the sample mean given the sample variance, we give a theorem that \DIFdelbegin \DIFdel{embodies a process slightly improved from }\DIFdelend \DIFaddbegin \DIFadd{is representative of a slightly improved derivation process than }\DIFaddend that followed by \cite{Maurer50empiricalbernstein}\DIFaddbegin \DIFadd{, where they use this process to create their EBB}\DIFaddend .
...
\DIFdelbegin \DIFdel{Assume }\DIFdelend \DIFaddbegin \DIFadd{For probability bounds $\p(\hat{\mu}-\mu>t)\le h(\sigma^2,t)$ and $\p(\sigma^2-\hat{\sigma}^2>w)\le f(\sigma^2,w)$,
if }\DIFaddend $f^{-(2)}$ and $h^{-(2)}$ both exist, and also if $h^{-(2)}$ is monotonically increasing in its first argument, so that we can define:
\[
...
%
\begin{proof}
Substituting $w$ for \DIFdelbegin \DIFdel{$f^{-2}(\sigma^2,w)$ }\DIFdelend \DIFaddbegin \DIFadd{$f^{-(2)}(\sigma^2,w)$ }\DIFaddend gives:
