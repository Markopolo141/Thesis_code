...
In the \DIFdelbegin \DIFdel{last }\DIFdelend \DIFaddbegin \DIFadd{previous }\DIFaddend chapter we introduced several different \DIFdelbegin \DIFdel{mathematical approaches to the problem of allocation and in }\DIFdelend \DIFaddbegin \DIFadd{solution concepts, each stemming from different ethical principles and perspectives.
In }\DIFaddend this chapter we \DIFdelbegin \DIFdel{introduce a new solution that builds upon some elements of the previous solutions.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Particularly we develop a solutionconcept that extends Nash bargaining with endogenous disagreement point (from }\DIFdelend \DIFaddbegin \DIFadd{detail a new approach that combines some of the features of the solution concepts outlaid in the previous chapter.
The primary motivation for this new solution, is to attempt to distil the intuition behind the Nash bargaining solution concepts (per }\DIFaddend section \ref{sec:solutions_bargaining}) \DIFdelbegin \DIFdel{to a solution concept accommodating arbitrary numbers of players }\DIFdelend \DIFaddbegin \DIFadd{into a description of total allocation that is suitable for arbitrary number of players - rather than just two}\DIFaddend .
Our solution extends the work of others to the space of generalised non-cooperative games, which are then suitable for \DIFdelbegin \DIFdel{describing }\DIFdelend \DIFaddbegin \DIFadd{application in }\DIFaddend various electrical network contexts.

\DIFaddbegin \DIFadd{Our solution relates to many of the concepts of the previous chapter: where we have a fundamentally coalitional scheme (per Coalitional Game Theory), that rewards based upon disagreement points (Bargaining Theory), which is partially informed by how much individual's participation influences the group's wellbeing (reminiscent of VCG), and attempts to describe normative trading between large numbers of participants (Marginalism).
}

\DIFaddend We call our new solution concept the \textit{Generalized Neyman and Kohlberg Value} or \textit{GNK value} for short, and is computed and \DIFaddbegin \DIFadd{directly }\DIFaddend compared against other solutions concepts of the previous chapter in the context of electricity allocation.
...
\DIFdelbegin \section{\DIFdel{Prelude to the GNK value}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{In the previous chapter we introduced several different solution concepts, each stemming from different ethical principles and perspectives.
}\DIFdelend \DIFaddbegin \DIFadd{This chapter consists of the following parts:
}\DIFaddend 

\DIFdelbegin \DIFdel{In this section we detail a new approach that combines some of the features of the solution concepts outlaid in the previous chapter.
...
%DIFDELCMD < %%%
\DIFdel{In the Nash bargaining solution concept, }\DIFdelend \DIFaddbegin \begin{itemize}
\item	\DIFadd{In sections \ref{sec:prelude_gnk} and \ref{the_value_def2}, we introduce and define the GNK value, relating it to historical roots and similar solution concepts.
}\item	\DIFadd{In section \ref{more_involved}, we consider how the GNK value can be computed to derive financial and electrical outcomes on a DC electricity network, against LMP and VCG.
}\item	\DIFadd{In section \ref{sec:features}, we point-by-point discuss the features GNK, LMP and VCG as they are expressed in }\DIFaddend the \DIFdelbegin \DIFdel{minimax payoff advantage between two parties is considered as a measure of bargaining strength between them, and so in our new solution we consider the minimax payoff advantages that could occur in bargaining between all possible coalitions of players.
...
}\DIFdelend \DIFaddbegin \DIFadd{context of an example electricity network.
%DIF > \item	in section \ref{sec:scaling} we address the possible ways of remedying the GNK value for computation in larger network contexts, particularly via the use of sampling to approximate its value, and by the adoption of a proxy for the GNK inner optimisation (which we denote as the M-GNK).
%DIF > \item	in section \ref{sec:results_and_evaluation_of_GNK}, we detail computation and the efficiency of computation of the sampled M-GNK value for larger networks, and discuss its shortcommings for an example computed larger network.
%DIF > \item	in section \ref{sec:GNK_extensions_discussion} and the concluding section \ref{sec:GNK_value_discussion}, we discuss possible extensions to the GNK concept and compare it point-by-point with ethical qualities.
}\end{itemize}
\DIFaddend 

\DIFdelbegin \DIFdel{Our approach straightforwardly relates to many of the concepts of the previous section: where we have a fundamentally coalitional scheme (per Coalitional Game Theory), that rewards based upon disagreement points (Bargaining Theory), which is partially informed by how much individual's participation influences the group's wellbeing (reminiscent of VCG), and attempts to describe normative trading between }\DIFdelend \DIFaddbegin \DIFadd{We identify that the GNK value is difficult to compute for }\DIFaddend large numbers of \DIFdelbegin \DIFdel{participants (Marginalism)}\DIFdelend \DIFaddbegin \DIFadd{players, and so in the next chapter \ref{sec:scaling} we address and consider the GNK value at scale against ethical criteria}\DIFaddend .

%DIF < much more than they are necessarily prescriptive outcomes of an idealised ethics.
%DIF < Though the two (ideal competitive outcomes and ethical outcomes) may be related, or even identical in certain contexts - such as where no particular ethical principles apply or the purpose of the engagement is the competition (such as in sports).
%DIF < And it is in this same way that the Marginalism outlined in Section \ref{} is often primarily treated usually begin descriptive than prescriptive.
%DIF > \begin{itemize}
%DIF > \item Derive the GNK value (section \ref{the_value_def2})
%DIF > \item Apply it to DC networks (section \ref{more_involved})
%DIF > \item We discuss some of its qualities (section \ref{sec:features})
%DIF > \item Before discussing techniques to scale it (section \ref{sec:scaling})
%DIF > \item Then evaluating it at scale (section \ref{sec:results_and_evaluation_of_GNK})
%DIF > \item And then conclude with an ethical evaluation (section \ref{sec:GNK_extensions_discussion})
%DIF > \end{itemize}


%DIF < However there is a line of ethical thinking that idealises competition as being associated-with or determining appropriate ethics; as we consider in Section \ref{}, and for those inclined to think that way then our solution is all the more relevant.
%DIF < In anycase, the process of crafting better (or different) descriptions of competitive outcomes is not to be feared, as it may be usefull for describing situations as they might occur under free competition, or as a starting point for implementation in arenas without or prior-to relevant ethical concerns.
...
\DIFdelbegin \DIFdel{Let us begin.
}\DIFdelend %DIF > Particularly:
%DIF > \begin{itemize}
%DIF > \item	in sections \ref{sec:prelude_gnk},\ref{sec:introduction_gnk_sec},\ref{the_value_def2}, We introduce and define the GNK value at a conceptual and mathematical level, relating it to historical roots and similar solution concepts.
%DIF > \item	in sections \ref{more_involved} we consider how the GNK value can be applied to derive financial and electrical transactions on a small example electricity network, and outlay the procedures for its computation alongside LMP and VCG in this context.
%DIF > \item	in section \ref{sec:features} we point-by-point discuss the features GNK, LMP and VCG as they are expressed in the context of the small example DC electricity network. particularly we pay attention to the fact that the GNK value is computationally difficult to compute for larger networks.
%DIF > \item	in section \ref{sec:scaling} we address the possible ways of remedying the GNK value for computation in larger network contexts, particularly via the use of sampling to approximate its value, and by the adoption of a proxy for the GNK inner optimisation (which we denote as the M-GNK).
%DIF > \item	in section \ref{sec:results_and_evaluation_of_GNK}, we detail computation and the efficiency of computation of the sampled M-GNK value for larger networks, and discuss its shortcommings for an example computed larger network.
%DIF > \item	in section \ref{sec:GNK_extensions_discussion} and the concluding section \ref{sec:GNK_value_discussion}, we discuss possible extensions to the GNK concept and compare it point-by-point with ethical qualities.
%DIF > \end{itemize}
...
%DIF > After this chapter, the next Chapter \ref{chap:stratified_sampling_chapter} details our investigation into novel sampling methods which are which are used to scale the GNK value in section \ref{sec:scaling}.
\DIFaddend 

\section{Introduction to the GNK value}\DIFaddbegin \label{sec:prelude_gnk}\label{sec:introduction_gnk_sec}
\DIFaddend 


How should we model ideal competition? In the two player case, existing bargaining solutions (such as Nash's) seem rather difficult to surpass as they describe a singular and axiomatic outcome that is both cooperatively \DIFdelbegin \DIFdel{pareto-optimal }\DIFdelend \DIFaddbegin \DIFadd{Pareto optimal }\DIFaddend and also accounts for anti-cooperative strategising.
...
However there exists a problem extending this scheme directly to three or more players, as there may not exist a unique minimax equilibrium in payoff-advantages for strategies in a game of \DIFdelbegin \DIFdel{three-or-more }\DIFdelend \DIFaddbegin \DIFadd{more than two }\DIFaddend players.
...
In \DIFaddbegin \DIFadd{this way our new solution allocates outcomes in proportion to the aggregate leverage that all the individuals - and groups of individuals - could hypothetically posses in bargaining for outcomes they desire.
In }\DIFaddend the following sections of this chapter (sections \ref{the_value_def2} to \ref{more_involved}) we give all the details of this process.% Particularly we note that this process of how to reasonably extend Nash's bargaining solution to multiple players has a history.
...
The details of this problem and our novel remedy are given in following \DIFdelbegin \DIFdel{sections}\DIFdelend \DIFaddbegin \DIFadd{section \ref{the_value_def3}}\DIFaddend .

But briefly, in electricity network contexts the mutual interactions of participants can be limited \DIFaddbegin \DIFadd{by the physical constraint of the network, for instance: network participants cannot simply draw or push power to/from the network without restraint, as doing so would lead to damage to the network}\DIFaddend .
...
This solution concept is shown to apply for all transferable utility (TU) generalised non-cooperative games\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{, and directly equivalent to the Nash bargaining with endogenous disagreement point under transferable utility (TU) between two players (such as per section \ref{sec:nash_bargaining_exogenous}, as in the example in that section).
}
...
Let us derive the GNK value \DIFaddbegin \DIFadd{(in section \ref{the_value_def2}) }\DIFaddend before giving details of its application \DIFaddbegin \DIFadd{to DC networks (in section \ref{more_involved}), discussing it (section \ref{sec:features}) and scaling it (section \ref{sec:scaling})}\DIFaddend .
...
\DIFaddbegin \DIFadd{In this way the GNK value can be seen as a conceptual continuation of historic solution concepts, and can be judged according to how well it derives outcomes in application contexts.
}

\DIFadd{As a simple example demonstration, it is possible to see how the GNK value is identical to Nash bargaining in the context of transferable utility games (such as in the example matrix game \ref{eq:example_game1} of section \ref{subsec:nash_bargaining_endogenous}), we calculate all the terms $v(S)$ for coalitions of players $1$ and $2$, particularly:
}

\begin{align*}
\DIFadd{v(\{1,2\})=}&\DIFadd{\max\max\begin{bmatrix}3 & -3\\ -3 & 3\end{bmatrix}=3}\\
\DIFadd{v(\{1\})=}&\DIFadd{\frac{1}{2}\min\max\begin{bmatrix}1 & 1\\ -1 & -1\end{bmatrix}+\frac{1}{2}\max\min\begin{bmatrix}1 & -1\\ 1 & -1\end{bmatrix}=1}\\
\DIFadd{v(\{2\})=}&\DIFadd{\frac{1}{2}\min\max\begin{bmatrix}1 & -1\\ 1 & -1\end{bmatrix}+\frac{1}{2}\max\min\begin{bmatrix}1 & 1\\ -1 & -1\end{bmatrix}=-1}\\
\DIFadd{v(\emptyset)=}&\DIFadd{-\max\max\begin{bmatrix}3 & -3\\ -3 & 3\end{bmatrix}=-3}\\
\DIFadd{}\end{align*}
\DIFadd{From this:
}$$ \DIFadd{\varphi_1(\langle N,v\rangle)=\frac{1}{2}(v(\{1,2\})+v(\{1\}))=\frac{1}{2}(3+1)=2 }$$
$$ \DIFadd{\varphi_2(\langle N,v\rangle)=\frac{1}{2}(v(\{1,2\})+v(\{2\}))=\frac{1}{2}(3-1)=1 }$$
\DIFadd{Which matches exactly the result in section \ref{subsec:nash_bargaining_endogenous}. In section \ref{subsec:nash_bargaining_endogenous} we considered that the minimax in the payoff advantage matrix (ie. $d$ in that section) was the nash equilibrium point of threat in bargaining between the players in choosing a subsequent point on the Pareto frontier (see Figure \ref{fig:graph1_utilities}) that divides the maximum possible sum revenue (ie $s$ in that section).
The GNK value also directly encodes this same logic as $v(\{1\})=-v(\{2\})$ is the minimax in the payoff advantage between the player (in this ungeneralised game), and $v(\{1,2\})$ is the maximum sum revenue which is split. This result is the same as we would get by the }\textit{\DIFadd{coco-value}}\DIFadd{, and working with Neyman and Kohlberg's value (equation \ref{knvalue2}).
}

\DIFadd{In the following section we describe the setup for an application and evaluation of the GNK value in the context of DC-approximated electrical networks.
}\DIFaddend 

\section{GNK value \DIFdelbegin \DIFdel{applied to }\DIFdelend \DIFaddbegin \DIFadd{computation on }\DIFaddend DC powerflows}\label{more_involved}

Because of its flexibility the GNK value has the potential to be used in a large range of different contexts,
however in this section we focus solely on the development of a simple case --- the pricing \DIFdelbegin \DIFdel{the immeditate }\DIFdelend \DIFaddbegin \DIFadd{of the immediate }\DIFaddend consumption and generation of power on a meshed network under DC approximation, where all participants have linear utilities over their own power.
...
In this section, we consider \DIFdelbegin \DIFdel{an example electricity network under DC approximation and show how the electrical consumption/generation of network participants can be modelled as a generalised game. And then give some details of a computational methods which can be used to compute }\DIFdelend \DIFaddbegin \DIFadd{the DC network model, discuss how to calculate }\DIFaddend the GNK value \DIFdelbegin \DIFdel{in this context}\DIFdelend \DIFaddbegin \DIFadd{as well as LMP and VCG for it, before in the next section \ref{sec:features} we discuss the features of these mechanisms with an example}\DIFaddend .
...
We further assume that there is a linear utility (or payoff) associated with the power consumption of each player, denoted $u_i(p_i)$ for player $i$, which \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{makes }\DIFaddend all the components of a generalised game.\DIFdelbegin \DIFdel{In the next subsection \ref{sec:example_network} we give a simple example of such a game.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection{\DIFdel{An example problem}}%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{sec:example_network}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{As an example instance, we consider the toy 5-bus network shown in Figure \ref{fig:example1}, with parameters given in Table~\ref{tab:example1}.
From this example it is possible to calculate and subsequently consider the features of }\DIFdelend \DIFaddbegin \footnote{\DIFadd{the Linear utility assumption simplifies the mathematics, as it makes the objective function linear, intuitively consumers should value consuming more power more, and generators cost producing more power more with some gradient.}}
\DIFadd{We now consider how to calculate }\DIFaddend the GNK value against LMP and VCG \DIFdelbegin \DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{To this end, in sections \ref{subsec:LMP_compute1}, \ref{subsec:VCG_compute1} and \ref{subsec:gnk_compute1} we outlay procedures used to calculate the financial payments and dispatched powers under the GNK value, LMP and VCG respectively, against parameter $p_1^l$ (the generator capacity in the network).
...
%DIFDELCMD < %%%
\DIFdel{We begin by describing the process of calculating the LMP values for this example network before considering VCG and then GNK calculation methodologies; in the order of complexity.
}%DIFDELCMD < 

%DIFDELCMD < \input{figs/Electro_table.tex}
%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection{\DIFdel{Computing the LMP transfers}}%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{subsec:LMP_compute1}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{To compute the Locational Marginal Price (LMP) transfers for our example network, we follow a process described in various literature - such as by \mbox{%DIFAUXCMD
\cite{lmp1,lmp2}}\hspace{0pt}%DIFAUXCMD
.
...
%DIFDELCMD < %%%
\DIFdel{We consider maximising the sum of utility: $\sum_{i\in B} u_i(p_i)$ subject to the DC powerflow constraints in equations \ref{dcopf1}.
...
%DIFDELCMD < %%%
\DIFdel{What is most notable about Locational Marginal Pricing, as witnessed here and elsewhere, is that it is computationally easier than other schemes as only one optimisation problem is needed to be solved.
...
%DIFDELCMD < %%%
\DIFdel{The SCIP optimisation suite}\footnote{\DIFdel{development reported by \mbox{%DIFAUXCMD
\cite{MaherFischerGallyetal.2017}}\hspace{0pt}%DIFAUXCMD
}} %DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{was employed for the optimisation of our example network, and the results of the calculation are shown in Figures \ref{fig:1f} and \ref{fig:1e}.
...
%DIFDELCMD < %%%
\subsection{\DIFdel{Computing the VCG imputations}}%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{subsec:VCG_compute1}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Unlike LMP, calculating VCG payments for our example network involves solving multiple optimisation problems.
...
%DIFDELCMD < %%%
\DIFdel{In this way the socially optimum value $x^*$ needs to be computed, and then additionally an additional optimisation problem for each player - where we assume that the excluded participant has power zero $p_i=0$. Thus in our context, equation \ref{eq:VCG_payment_rule} becomes:
}\begin{displaymath} \DIFdel{d_i(v)=\argmax_{\sum_{j} u_j(p_j)}\sum_{j\ne i}u_j(p_j) - \argmax_{\sum_{j\ne i} u_j(p_j), p_i=0}\sum_{j\ne i}u_j(p_j)}\end{displaymath}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The first term in this equation is the sum of utilities at the point which maximises the sum of utilities minus the player $i$.
...
The discussion and comparison of these figures is given in section \ref{sec:features}}\DIFdelend \DIFaddbegin \DIFadd{for such a generalised game}\DIFaddend .
...
This method was chosen, and the SCIP Optimisation Suite was employed to compute the GNK value for \DIFdelbegin \DIFdel{our }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend example network as described in the \DIFdelbegin \DIFdel{previous }\DIFdelend \DIFaddbegin \DIFadd{next }\DIFaddend section \ref{sec:example_network}.

KKT conditions are a well known set of algebraic tests which imply that the function under consideration is locally optimal (maximal or alternatively minimal) with respect to its variables under a set of constraint functions (with some regularity assumptions on \DIFdelbegin \DIFdel{thoes }\DIFdelend \DIFaddbegin \DIFadd{those }\DIFaddend functions).
...
It was also realised that a binary reformulation of the complementary slackness conditions (equation \ref{eq:complementarity_constraint_KKT}) would increase computational efficiency, and so we transformed these complementarity constraints into \DIFdelbegin \DIFdel{disjunnctive }\DIFdelend \DIFaddbegin \DIFadd{disjunctive }\DIFaddend binary constraints.
...
the resulting KKT conditions for maximising and objective function $f(p_{i\in B})$ under this complementary \DIFdelbegin \DIFdel{slcakness }\DIFdelend \DIFaddbegin \DIFadd{slackness }\DIFaddend conditions are as follows:
...
%DIF > Using the SCIP optimisation suite the GNK transfers were calculated for an example DC network in the next section \ref{sec:example_network} particularly as shown in Figures \ref{fig:1d} and \ref{fig:1c} show the utility transfers and the post-transfer utilities under GNK.


\DIFdelbegin \DIFdel{Using }\DIFdelend \DIFaddbegin \subsection{\DIFadd{Computing the LMP transfers}}\label{subsec:LMP_compute1}

\DIFadd{Computing the Locational Marginal Price (LMP) transfers for DC electricity networks is a process described in various literature - such as by \mbox{%DIFAUXCMD
\cite{lmp1,lmp2}}\hspace{0pt}%DIFAUXCMD
.
}

\DIFadd{Particularly we consider optimising the sum of utility: $\sum_{i\in B} u_i(p_i)$ subject to the DC powerflow constraints in equations \ref{dcopf1}.
In this context the power conservation constraints on each bus are the constraints $p_j = \sum_{(i,j)\in C}p_{i,j}$, and the lagrange multipliers associated with these constraints are the marginal prices for power on each of the respective busses.
...
We utilised }\DIFaddend the SCIP optimisation suite\DIFdelbegin \DIFdel{, the GNK transfers we calculated, and in Figures \ref{fig:1d} and \ref{fig:1c} show the utility transfers and the post-transfer utilities under GNK; we discuss these figures next}\DIFdelend \DIFaddbegin \footnote{\DIFadd{development reported by \mbox{%DIFAUXCMD
\cite{MaherFischerGallyetal.2017}}\hspace{0pt}%DIFAUXCMD
}}\DIFadd{.
}

\subsection{\DIFadd{Computing the VCG imputations}}\label{subsec:VCG_compute1}

\DIFadd{From section \ref{sec:solutions_VCG} the VCG payment that a participant makes is the difference between the sum of other's utility at the social optimum point $x^*$, and the utility that others would have if the participant were not present and the optimisation were only over the remaining participants, as seen in equation \ref{eq:VCG_payment_rule}.
}

\DIFadd{In this way the socially optimum value $x^*$ needs to be computed, and then additionally an additional optimisation problem for each player - where we assume that the excluded participant has power zero $p_i=0$. Thus in our context, equation \ref{eq:VCG_payment_rule} becomes:
}\begin{equation}\DIFadd{\label{eq:VCG_calculation_DC} d_i=\argmax_{\sum_{j} u_j(p_j)}\sum_{j\ne i}u_j(p_j) - \argmax_{\sum_{j\ne i} u_j(p_j), p_i=0}\sum_{j\ne i}u_j(p_j)}\end{equation}

\DIFadd{The first term in this equation is the sum of utilities at the point which maximises the sum of utilities minus the player $i$.
The second term in this equation is the sum of other's utilities (ie. excluding player $i$) at the point which maximises the sum of other's utilities in the context that the players power is zero.
The first part of the equation is common to all player's contributions $d_i$, but the second negative part is unique for each.
In this way, if there are $n$ participants, there are $n+1$ comparable OPF optimisation problems which need to be solved to calculate VCG payments.
...
\subsection{\DIFadd{Computing the Shapley Value imputations}}\label{subsec:Shap_compute1}

\DIFadd{From section \ref{subsec:the_shapley_value} the Shapley Value utility imputations that a participant receives is the average marginal contribution that it adds to a characteristic function under ambiguity of the join ordering.
In this way we need to consider a characteristic function in the context of DC networks:
}\begin{equation*}\DIFadd{v_{shap}(S) = \max\sum_{j}u_j(p_j)~~~~\text{s.t.}~~\forall i\notin S~~p_i=0}\end{equation*}
\DIFadd{This characteristic function describes the utility that a coalition could achieve by themselves, absent any consumption/generation from those not in the coalition - it is one possible way of creating a characteristic function for DC networks.
}

\DIFadd{The utilities under the Shapley Value (by efficiency axiom) sum to give the value of the grand coalition $v_{shap}(N)$ which occurs at the optimal operating point $x^*$.
...
To calculate the Shapley Value imputations, the maximal sum of utilities under OPF for all the possible $2^n-1$ coalitions need computed, and the SCIP optimisation suite was used for these calculations}\DIFaddend .




\section{Some features of GNK\DIFaddbegin \DIFadd{, in context of an example}\DIFaddend }\label{sec:features}\DIFaddbegin \label{sec:example_network}
\DIFaddend 

In \DIFaddbegin \DIFadd{the previous section \ref{more_involved} we detailed a procedure to calculate the financial payments and dispatched powers under the GNK value as well as LMP, VCG and Shapley Value, so that now we can compute and compare them with an example.
In }\DIFaddend this section we \DIFaddbegin \DIFadd{are thus able to witness and }\DIFaddend discuss the characteristics of the GNK value, \DIFdelbegin \DIFdel{against LMP and VCG, for the }\DIFdelend \DIFaddbegin \DIFadd{in the context of an }\DIFaddend example 5-bus network \DIFdelbegin \DIFdel{(from Figure ~\ref{fig:example1}).
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{For GNK , LMPand VCG, the power allocations and the utilities associated with these power allocations are plotted against }\DIFdelend \DIFaddbegin \DIFadd{shown in Figure \ref{fig:example1}, with parameters given in Table~\ref{tab:example1}.
In this example we calculate and subsequently consider the features of the GNK value against LMP, VCG and Shapley Value against parameter }\DIFaddend $p_1^l$ (the generator capacity in the network)\DIFdelbegin \DIFdel{and are shown in Figures \ref{fig:1a} and \ref{fig:1b}.
And through subsequent pairs of figures we can see how the different mechanisms allocate payments and utilities in the context of constrained resources - Figures \ref{fig:1c}, \ref{fig:1d}for GNK, Figures \ref{fig:1e}, \ref{fig:1f}for LMP, }\DIFdelend \DIFaddbegin \DIFadd{.
The results of these calculations of the financial payments are plotted against $p_1^l$ in }\DIFaddend Figures \DIFdelbegin \DIFdel{\ref{fig:1g}, \ref{fig:1h} for VCG}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:1d}, \ref{fig:1f}, \ref{fig:1h} and \ref{fig:1j} for GNK, LMP, VCG and Shapley Value respectively. Which in addition to the utilities derived from power consumption/generation (Figure \ref{fig:1b}) form the post payment utility imputations as Figures \ref{fig:1c}, \ref{fig:1e}, \ref{fig:1g} and \ref{fig:1i}}\DIFaddend .

%DIF > For GNK, LMP and VCG, the power allocations and the utilities associated with these power allocations are plotted against $p_1^l$ (the generator capacity in the network) and are shown in Figures \ref{fig:1a} and \ref{fig:1b}.
%DIF > And through subsequent pairs of figures we can see how the different mechanisms allocate payments and utilities in the context of constrained resources - Figures \ref{fig:1c}, \ref{fig:1d} for GNK, Figures \ref{fig:1e}, \ref{fig:1f} for LMP, Figures \ref{fig:1g}, \ref{fig:1h} for VCG.
\DIFaddbegin 
...
VCG takes into account the marginal effect that each of the player's participation has \DIFdelbegin \DIFdel{apon }\DIFdelend \DIFaddbegin \DIFadd{upon }\DIFaddend each other at the social optimum
...
\DIFdelbegin \DIFdel{From these graphs we can witness some of the qualities of }\DIFdelend \DIFaddbegin \DIFadd{In contrast to VCG, LMP and GNK, the Shapley Value exhibits character traits that are similar to LMP and VCG, particularly as the Shapley value is composed of many more marginal contributions than those ones inherent to VCG, which is in turn proportional to the marginalism inherent in LMP.
Furthermore it has a superficial similarity to }\DIFaddend the GNK value \DIFdelbegin \DIFdel{against LMP and VCG}\DIFdelend \DIFaddbegin \DIFadd{as they both extend from the same axioms - either exhibited in section \ref{the_value_def}, or by conversion to Shapley value by equation \ref{convert1} thus having exactly the same axioms as in section \ref{subsec:the_shapley_value}.
The Shapley Value results are plotted against $p_1^l$ for our example in Figure~\ref{fig:1i}, and the resulting transfers (GNK value less utility) are plotted in Figure~\ref{fig:1j}}\DIFaddend .


\DIFdelbegin \subsection{\DIFdel{Specific features of the GNK value}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend From these graphs we can witness \DIFdelbegin \DIFdel{that LMP, VCG and GNK values feature a range of different dynamics,
but some particularly notable }\DIFdelend \DIFaddbegin \DIFadd{some of the }\DIFaddend qualities of the GNK value \DIFdelbegin \DIFdel{include}\DIFdelend \DIFaddbegin \DIFadd{against LMP and VCG and the Shapley Value}\DIFaddend :

% \begin{itemize}
% \item GNK is continuous in the parameters of the network
% \end{itemize}

\subsubsection*{The GNK value\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend VCG \DIFaddbegin \DIFadd{and Shapley Value }\DIFaddend imputations are continuous in the parameters of the network}
...
This \DIFaddbegin \DIFadd{happens because the change in generator capacity is changing the parameters of the network itself and its feasibility region, which yields discontinuous changes in network operating point, and thus marginal prices.
This dynamic under LMP }\DIFaddend might be seen to lead to a somewhat perverse incentive to produce less power than what is socially optimal, and in contrast, the utilities under the GNK value (Figure \ref{fig:1c})\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend VCG (Figure \ref{fig:1g}) \DIFaddbegin \DIFadd{and Shapley Value (Figure \ref{fig:1i}) which }\DIFaddend feature no such drops or discontinuities.
...
In contrast, the post-payment utilities under the GNK value and VCG are \DIFdelbegin \DIFdel{almost }\DIFdelend always continuous with network parameters.
...
% \end{itemize}
\subsubsection*{The GNK \DIFaddbegin \DIFadd{and Shapley Value }\DIFaddend payments are always budget balanced}
The transactions under \DIFdelbegin \DIFdel{LMP and }\DIFdelend VCG are not necessarily budget-balanced and can yield a surplus or deficit, \DIFaddbegin \DIFadd{and transactions under LMP can yield a budget surplus (they are weakly budget balanced \mbox{%DIFAUXCMD
\cite{Wu1996}[Fact 4]}\hspace{0pt}%DIFAUXCMD
), }\DIFaddend whereas payments under the GNK value \DIFdelbegin \DIFdel{are necessarily }\DIFdelend \DIFaddbegin \DIFadd{and Shapley Value are strongly }\DIFaddend budget-balanced and result in no surplus or deficit.
This is an outcome of the GNK value's axiomatic derivation, as given by equation \eqref{myeq}\DIFaddbegin \DIFadd{, which is reflective of the Shapley Value axioms, per section \ref{subsec:the_shapley_value}}\DIFaddend .
...
The surplus of $\$420$ comes particularly from the existence of congestion in the example network which is well known to introduce so-called \DIFdelbegin \DIFdel{``}\DIFdelend \DIFaddbegin \DIFadd{`}\DIFaddend congestion-rents\DIFdelbegin \DIFdel{'' that are usually collected by owners of transmission lines (and which are often also the owners of the utility-scale generation assets in vertically-integrated systems - see \mbox{%DIFAUXCMD
\cite{lmp2}}\hspace{0pt}%DIFAUXCMD
...
\DIFaddbegin \DIFadd{Under Shapley Value, it is witnessed that there is some rather small offset for those that do not receive power, though this is mostly determined by marginal coalitional considerations that exclude most other consumers.
}\DIFaddend 
...
Unlike VCG, the GNK value\DIFdelbegin \DIFdel{and LMP }\DIFdelend \DIFaddbegin \DIFadd{, LMP and Shapley Value }\DIFaddend are not \emph{incentive compatible} in the sense that is often referred-to in Mechanism Design (see Section \ref{sec:solutions_VCG}).
...
In the GNK value this can be seen in \eqref{knvalue1}, or more easily in its reduced form, \eqref{knvalue2}, where the payoff advantage of a coalition $v(S)$ is based on its reported utilities in minimax strategies which may not be actualised\DIFdelbegin \DIFdel{.
And hence, misreporting these utilities }\DIFdelend \DIFaddbegin \DIFadd{; the same consideration holds for the Shapley Value.
Because of this consideration, misreporting the utilities of these unactualised events }\DIFaddend may change the $v(S)$ and hence the GNK value itself.
...
% \end{itemize}
\subsubsection*{The GNK value \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{and Shapley Value are }\DIFaddend computationally difficult}
The GNK value is more difficult than \DIFdelbegin \DIFdel{VCG and significantly }\DIFdelend \DIFaddbegin \DIFadd{the Shapley Value, which is more difficult than VCG, which is }\DIFaddend more difficult than LMP, to compute.
...
\DIFdelbegin \DIFdel{This computational difficulty }\DIFdelend \DIFaddbegin \DIFadd{The Shapley Value also requires calculating $v_{shape}(S)$ for all the $2^n-1$ possible coalitions $S$, but the calculation of $v_{shape}(S)$ is not necessarily NP-hard problem, as DC OPF problem can be a linear optimisation.
...
\DIFadd{The computational difficulty of the GNK value }\DIFaddend is reflected in a later Figure \ref{fig:performance_graph2} where it is seen that even using sampling to approximate the GNK value \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{to sufficient accuracy is witnessed to be a }\DIFaddend double-exponentially complex \DIFdelbegin \DIFdel{. 
This difficulty contrasts against the ease of computing LMPwhich requires only solving one OPF optimisation, and VCG which requires $n+1$ OPF optimisations}\DIFdelend \DIFaddbegin \DIFadd{process. 
}


\section{\DIFadd{Summary}}

\DIFadd{In this Chapter we have introduced the GNK value from its axioms and we have considered its conceptual inheritance from other solution concepts.
...
We then compared the GNK against LMP, VCG and Shapley Value on a small example network to examine their features, particularly we identified that the GNK value has some nice budget-balance and continuity properties, however we also realised that the GNK is not incentive compatable and that it is difficult to calculate for larger electricity networks}\DIFaddend .

In \DIFdelbegin \DIFdel{light of this difficulty, we consider different ways of mitigating the issue so that }\DIFdelend \DIFaddbegin \DIFadd{the next chapter we consider the ways in which the computational difficultly of calculating }\DIFaddend the GNK value \DIFdelbegin \DIFdel{can be applied to larger networks}\DIFdelend \DIFaddbegin \DIFadd{on larger networks can be ameliorated.
...
We discuss these to directions in the next chapter \ref{sec:scaling} before actually applying these techniques for an application of GNK to larger network in section \ref{sec:results_and_evaluation_of_GNK} where we discuss in section \ref{sec:GNK_value_discussion}}\DIFaddend .
