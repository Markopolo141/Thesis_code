...
This process of using EBBs involved binding EBBs applied to different strata together using union bounds to create a bound on the stratified sample mean error (via \DIFdelbegin \DIFdel{with }\DIFdelend Theorem \ref{triangle_theorem1}).
...
It is noted that the triangle equality $|A+B|\le |A|+|B|$, is only an equality in the event that the elements $A$ and $B$ are of the same sign, and in the context of theorems \ref{triangle_theorem1} and \ref{triangle_theorem2} the bound for the error is developed \DIFaddbegin \DIFadd{by }\DIFaddend effectively assuming all the errors of the estimates of the strata are additive - which is \DIFdelbegin \DIFdel{positively }\DIFdelend the worst case.
...
\DIFdelbegin \DIFdel{Using this knowledge , stronger bounds are possible - }\DIFdelend \DIFaddbegin \DIFadd{By using this knowledge stronger bounds can be created, }\DIFaddend and in this section \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{we do just that. We have }\DIFaddend created an empirical (ie. depending on sample variances) concentration inequality specifically for stratified random sampling\DIFdelbegin \DIFdel{; and which then can be used to serve as a basis for choosing samples in the next section}\DIFdelend .
...
\DIFaddbegin \DIFadd{We note in this section, all proofs and demonstrations are our own, and are novel, except where otherwise explicitly noted.
}\DIFaddend 
...
We note that this process of fitting a parabola over the exponential function is exactly the same process as used to derive Bennett's inequality (Theorem \ref{hoeffdings1})\DIFdelbegin \DIFdel{.
But that this form of the bound on the moment generating function is }\DIFdelend \DIFaddbegin \DIFadd{, but that we derive }\DIFaddend a weakened result \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{from }\DIFaddend that same approach.
...
\end{theorem}
\begin{proof}
In a similar \DIFdelbegin \DIFdel{was as per }\DIFdelend \DIFaddbegin \DIFadd{way as }\DIFaddend Lemma~\ref{chernoff1} we consider a bound for the weighted mean by the moment generating function of the stratum means:
...
\subsection{Sequential sampling using the stratified empirical bernstein method\DIFdelbegin \DIFdel{(SEBM)}\DIFdelend } \label{sec:SEBMalgorithm}
